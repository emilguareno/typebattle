...
class FriendlyChat {
    ...
    loadNotificationTokens(){
        var addNotificationToken = data => {
            this.notificationTokens.push(data.getKey());
        };
        this.database.ref('/fcmTokens').on('child_added', addNotificationToken);
        this.database.ref('/fcmTokens').on('child_changed', addNotificationToken);
    }
    saveMessage(e) {
        e.preventDefault();
        ...
        if (this.messageInput.value && this.checkSignedInWithMessage()) {
            ...
            this.messagesRef.push({
                ...
                this.notifyUsers(this.messageInput.value);
    ...
    setImageUrl(imageUri, imgElement) {
        if(imageUri.startsWith('gs://')){
            imgElement.src = this.LOADING_IMAGE_URL; //Display loading image first.
    ...
    notifyUsers(message){
        ...
        xhr.setRequestHeader('Authorization', 'key=AAAAzUaCMJI:APA91bEhCAYX57K04iZjeJZsExHsh6zPRXCGdU4RErZ5zacAmY2IQN26emkUZnOKwLtzB7cUnOqcM3ERPQKaycqeNEkLUpVUweDOR_pNxAYu_ezlLVBhwBVKLDYO27-2ub_DcvoNY_WJ');
        xhr.send(JSON.stringify({
            ...
            registration_ids: this.notificationTokens
        }));
    }
    ...
    saveMessagingDeviceToken() {
        firebase.messaging().getToken().then((currentToken) => {
            if(currentToken){
                ...
                //Sets the user ID as the value of the token key.
